...
export function LanguageSelectorApple({ isDark }: { isDark: boolean }) {
  ...
  return (
    <div
      ...
      className={`inline-flex h-8 md:h-9 items-center rounded-full p-0.5 md:p-1 ${isDark ? 'bg-white/10' : 'bg-black/5'}`}
    >
      {supportedLocales.map((opt) => {
        ...
        return (
          <button
            ...
            className={`press compact-touch h-7 md:h-8 min-w-[34px] px-2 md:px-2.5 rounded-full text-[11px] font-semibold tracking-wide ${
              active
                ? isDark
