...
// roles), by tool name (free text). State lives in the URL (`?tools=<group>,<group>&surface=<id>
// &q=<text>`) so a filtered view is a link a visitor can share or bookmark — read once on mount,
// written with `replaceState` so filtering never grows browser history.
// 2026-09-11 — owner feedback: "fewer filters, clearer" (the min-stores threshold, the sort order and
// the active-filter chips row are gone; three controls remain — group, surface, name).
