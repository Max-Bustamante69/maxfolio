// Mobile candidate (d) — "Constellation": `?skillsMobile=d`. Mechanism: a static, spatial map instead
// of a list — six group clusters on a 2×3 grid, each a small dot scatter sized by usage (no rotation;
// the orbit's own comment on this file explains why an ambient-motion ring never fits a phone width).
// Tapping a cluster is a shared-element "magic move" (`layoutId`) into a full-width panel listing its
// real tools; a back arrow returns to the map. The dots themselves are decorative at this scale — only
// the cluster (44px+) and, once zoomed, each tool row are real tap targets.
