// Owner-only comparison switcher: only rendered when the page loaded with `?review=` present at all,
// so visitors never see it. Same pattern as featuredLayouts/VariantSwitcher.tsx.
...
export function VariantSwitcher({ skin, active, onSelect }: VariantSwitcherProps) {
  return (
    <div role="group" aria-label="Review checklist variant switcher (preview only)" className={`-mx-4 mb-6 flex flex-wrap gap-1.5 border-b px-4 pb-4 text-xs md:mx-0 md:px-0 ${skin.line}`}>
