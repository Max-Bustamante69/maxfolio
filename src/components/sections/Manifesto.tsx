...
interface ManifestoProps {
  skin: Skin
  /** Overrides the inverted band's background+text classes (defaults to `skin.invertedBand`). */
...
export function Manifesto({ skin, bandClassName, eyebrowClassName, backdropSrc }: ManifestoProps) {
  ...
  const band = bandClassName ?? skin.invertedBand
  const eyebrow = eyebrowClassName ?? skin.invertedAccent
    ...
    >
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <ul className="mt-6 space-y-2 md:space-y-3">
          {mf.lines.map((line, i) => (
            <li key={line} className={`${skin.headingFont} text-3xl font-semibold leading-[1.08] tracking-[-0.03em] md:text-5xl lg:text-6xl`}>
